...
```
- `GEO_TMPL_TYPE` - sets the type for the template header. This should be the types your geometries and points use. Typically, this will be either `double` or `float` but integer types are also available (`int` or `long` are tested).
...
- `GEO_TEST` - currently unused but is set during the unit test compilation. This allows for any test specific behavior.

#### Double/Float
There is 1 addition macro that is required to be set along with the `GEO_TMPL_TYPE` macro for floating points usage. Due to how comparisons are done between floating points numbers, the `GEO_TMPL_TYPE_SIZE` macro is required to be set. This value should be set to the bit size of the floating point being used.
...
5. Write unit tests for effected header file when changes are made. This means that an edit to the `include/geometry.h` file will require a unit test in ALL of the test files.
